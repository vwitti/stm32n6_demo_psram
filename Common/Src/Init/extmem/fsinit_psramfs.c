...
bool MX_EXTMEM_init_ps_ramfs(void)
{
	UINT ret = MX_FileX_Init_PSRAM_DISK_MM(FX_GetBytePoolDesc());
    if (ret != FX_SUCCESS)
    {
    	StartScreenLcd_WriteLine("PS-RAMFS: failed to mount (Map mode)");
    	HAL_Thread_SystemStop();
    }
    StartScreenLcd_WriteLine("PS-RAMFS: device mounted (Map mode)");
	return true;
}
