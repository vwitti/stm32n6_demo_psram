...
UCHAR *sram_disk_end = ((UCHAR *)(FX_SRAM_DISK_BASE_ADDRESS + FX_SRAM_DISK_SIZE));
    ...
    switch (media_ptr->fx_media_driver_request)
    {
        ...
        case FX_DRIVER_READ:
        {
            ...
            {
              /* Read sector from SPI Flash and copy the sector into the destination.  */
              uint32_t addr = (uint32_t) source_buffer;

          	  EXTMEM_StatusTypeDef ret_extmem = EXTMEM_Read(EXTMEM_SPI_FLASH, addr, (uint8_t *)media_ptr -> fx_media_driver_buffer, media_ptr->fx_media_driver_sectors * media_ptr->fx_media_bytes_per_sector);
          	  ...
          	  //printf("FX_DR: %08lX %ld res=%d\n", addr, media_ptr->fx_media_driver_sectors * media_ptr->fx_media_bytes_per_sector, ret_extmem);
        ...
        case FX_DRIVER_BOOT_READ:
        {
            ...
            bytes_per_sector =  512; //_fx_utility_16_unsigned_read(&source_buffer[FX_BYTES_SECTOR]);
            ...
            /* Read boot sector from SPI Flash and copy the sector into the destination.  */
            uint32_t addr = (uint32_t) source_buffer;

        	EXTMEM_StatusTypeDef ret_extmem = EXTMEM_Read(EXTMEM_SPI_FLASH, addr, (uint8_t *)media_ptr -> fx_media_driver_buffer, bytes_per_sector);
			  if(ret_extmem == EXTMEM_OK)
			  {
				/* Successful driver request.  */
				media_ptr->fx_media_driver_status = FX_SUCCESS;
        	...
        	//printf("FX_DR: %08lX %d res=%d\n", addr, bytes_per_sector, ret_extmem);
