...
UINT MX_USBX_Host_Init(VOID *memory_ptr)
{
  ...
  if (tx_queue_create(&ux_app_Host_MsgQueue_UCPD, "Message Queue UCPD", TX_1_ULONG,
  ...
  while (1)
  {
    ...
    else if (USB_DRD_State_Msg.HostState == STOP_USB_HOST)
    {
      ...
      if (hhcd_USB1_OTG_HS.State == HAL_HCD_STATE_READY)
      {
        ...
        USB_DisableGlobalInt(hhcd_USB1_OTG_HS.Instance);

      }

      HAL_HCD_DeInit(&hhcd_USB1_OTG_HS);
